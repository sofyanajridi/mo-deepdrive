...
from deepdrive_zero.discrete_actions.comfortable_actions import COMFORTABLE_ACTIONS, \
...
from deepdrive_zero.discrete_actions.comfortable_actions2 import \
...
from deepdrive_zero.discrete_actions.comfortable_steering_actions import \
...
class Agent:
    def __init__(self,
        ...
        self.env_variation = env_variation
        ...
        if self.multi_objective:
            if self.env_variation == 'OneWayPoint':
                self.episode_reward = np.array([0, 0, 0, 0], dtype="float64")
            elif self.env_variation == 'StaticObstacle':
                self.episode_reward = np.array([0, 0, 0, 0, 0], dtype="float64")
            elif self.env_variation == 'Intersection':
    ...
    def get_reward(self, won: bool, lost: bool,
        ...
        if self.multi_objective:
            if self.env_variation == 'OneWayPoint':
                ret = np.array([speed_reward, win_reward, self.gforce, self.jerk_magnitude])
            elif self.env_variation == 'StaticObstacle':
                ret = np.array([speed_reward, win_reward, self.gforce, collision_penalty, self.jerk_magnitude])
            elif self.env_variation == 'Intersection':
