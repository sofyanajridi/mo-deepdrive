...
from momarl_benchmarks.algorithms.so_dqn import DQN,multi_agent_train_DQN
...
wandb_name = "MA_SO_DQN_OneWaypointEnv_Utility_1"

vehicle_1_dqn_agent = DQN(env, batch_size=BATCH_SIZE, gamma=GAMMA, tau=TAU, lr=LR, utility_f=linear_utility_f,vectorial_reward=True)
vehicle_2_dqn_agent = DQN(env, batch_size=BATCH_SIZE, gamma=GAMMA, tau=TAU, lr=LR, utility_f=linear_utility_f,vectorial_reward=True)

multi_agent_train_DQN(vehicle_1_dqn_agent, vehicle_2_dqn_agent, env, 100_000, enable_wandb_logging="online",
                  wandb_group_name="MA_SO_DQN_OneWaypointEnv_Utility_1", wandb_name=wandb_name,config=env_config)
