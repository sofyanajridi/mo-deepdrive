...
#
#     return (0.50 * distance_reward) +  (win_reward) - (2 * collision_penalty) - (0.03 * gforce) - (0.00003 * jerk)

def linear_utility_f(vec):
    distance_reward, win_reward, gforce, collision_penalty, jerk, lane_penalty = vec

    return (0.50 * distance_reward) +  (win_reward) - (4 * collision_penalty) - (0.03 * gforce) - (0.00003 * jerk)
...
wandb_name = "MO_A2C_StaticObstacleEnv_Utility_1_" + dt_string

a2c_agent = MOA2C(env, LR, GAMMA, linear_utility_f)
a2c_agent.train(50_000, enable_wandb_logging="online", wandb_group_name="MO_A2C_StaticObstacleEnv_Utility_1", wandb_name=wandb_name)