...
def utility_f(vec):
    ...
    if distance_reward > 0:
        return  distance_reward + (win_reward) - collision_penalty - torch.pow((0.1 * gforce), 2) - torch.pow((0.1 * jerk), 2)
    else:
        return (0.50 * distance_reward) + (win_reward) - collision_penalty
...
wandb_name = "A2C_OneWaypointEnv_Utility_2_" + dt_string

a2c_agent = A2C(env, LR, GAMMA, utility_f,vectorial_reward=True)
a2c_agent.train(5000, enable_wandb_logging="online", wandb_group_name="SO_A2C_OneWaypointEnv_Utility_2", wandb_name=wandb_name)