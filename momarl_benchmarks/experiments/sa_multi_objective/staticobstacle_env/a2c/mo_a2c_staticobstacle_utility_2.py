...
def utility_f(vec):
    ...
    if distance_reward == 0:
        ...
        return (1 * distance_reward) + (win_reward) - (4 * collision_penalty) - torch.pow((0.03 * gforce),4) - torch.pow((3.3e-5 * jerk),4)
    else:
        return (0.50 * distance_reward) + (win_reward) - (4 * collision_penalty) -  torch.pow((0.03 * gforce),2) -  torch.pow((0.03 * gforce),2)



wandb_name = "MO_A2C_StaticOnstacleEnv_Utility_2"

a2c_agent = MOA2C(env, LR, GAMMA, utility_f)
a2c_agent.train(10_000, enable_wandb_logging="online", wandb_group_name="MO_A2C_StaticOnstacleEnv_Utility_2", wandb_name=wandb_name)