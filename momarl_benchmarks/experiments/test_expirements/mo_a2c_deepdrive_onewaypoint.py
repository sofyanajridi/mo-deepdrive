...
actor = Actor(state_dim + reward_dim, n_actions)
critic = Critic(state_dim + reward_dim, reward_dim)
...
import wandb
wandb.init(project="momarl-benchmarks",name="MO_A2C_DeepDrive_OneWayPoint", config=config, group="MO_A2C_DeepDrive_OneWayPoint",mode="disabled")
...
for episode in range(5000):
    ...
    while not done:
        probs = actor(torch.from_numpy(np.concatenate((obs, accrued_reward))).float())
        ...
        sum1 = utility(torch.Tensor(accrued_reward) + pow(gamma, timestep) * (
            ...
            torch.from_numpy(np.concatenate((next_obs, (accrued_reward + (pow(gamma, timestep) * reward))))).float())))
