...
class A2C:
    ...
    def __init__(self, env, lr, gamma, utility_f=None, vectorial_reward=False):
        ...
        self.actor_optimizer = torch.optim.RMSprop(self.actor_nn.parameters(), lr=self.lr)
        self.critic_optimizer = torch.optim.RMSprop(self.critic_nn.parameters(), lr=self.lr)
    ...
    def train(self, nr_episodes, enable_wandb_logging="disabled", wandb_group_name=None, wandb_name=None,
        ...
        for episode in range(nr_episodes):
            ...
            while not done:
                probs = self.actor_nn(torch.from_numpy(obs).float().to(device))
...
def multi_agent_train(vehicle_1: A2C, vehicle_2: A2C, env, nr_episodes, enable_wandb_logging="disabled",
    ...
    vehicle_1_reaches_destination = []
    vehicle_2_reaches_destination = []
