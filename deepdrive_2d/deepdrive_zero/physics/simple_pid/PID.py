...
from deepdrive_2d.deepdrive_zero.constants import CACHE_NUMBA
...
def call_pid(auto_mode, _last_output, _last_time, sample_time, setpoint,
             input_, _last_input, proportional_on_measurement, _proportional,
             output_limits, Kp, Ki, Kd, _integral):
    if not auto_mode:
        return _last_output

    now = _current_time()
    dt = now - _last_time if now - _last_time else 1e-16

    if sample_time is not None and dt < sample_time and _last_output is not None:
    ...
    d_input = input_ - (
        _last_input if _last_input is not None else input_)

    # compute the proportional term
    if not proportional_on_measurement:
        # regular proportional-on-error, simply set the proportional term
        _proportional = Kp * error
    else:
        # add the proportional error on measurement to error_sum
        _proportional -= Kp * d_input
        _proportional = _clamp(_proportional, output_limits)

    # compute integral and derivative terms
    _integral += Ki * error * dt
    _integral = _clamp(_integral, output_limits)  # avoid integral windup

    _derivative = -Kd * d_input / dt

    # compute final output
    output = _proportional + _integral + _derivative
    output = _clamp(output, output_limits)
